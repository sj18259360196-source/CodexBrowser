...
import { resolveBrowserRuntime } from "../browser/runtime-selection";
import { loadRuntimeSettings, resolveCodexBrowserProductRoot } from "../browser/runtime-settings";
import { MCP_SERVER_VERSION } from "../shared/release-info.js";
...
const TEST_MODE = process.env.CODEX_BROWSER_TEST_MODE === "1";
...
function timeoutForMethod(method: string): number {
  ...
  if (method === "browser.navigate") return 55_000;
  return 35_000;
}
...
function launchDesktop(): void {
  ...
  const edgeBrokerEntry = path.join(projectRoot, "dist", "browser", "edge-broker.mjs");
  const launchExecutable = useExternalEdge ? process.execPath : electronExecutable;
  const launchArguments = useExternalEdge ? [edgeBrokerEntry] : [projectRoot];
  if (!existsSync(useExternalEdge ? edgeBrokerEntry : electronExecutable)) {
    lastLaunchError = useExternalEdge
      ? "The external Edge runtime is not built. Run npm run build in the Codex Browser project."
      : "The Electron legacy runtime is unavailable. Run npm install and npm run build in the Codex Browser project.";
  ...
  try {
    child = spawn(launchExecutable, launchArguments, {
      cwd: projectRoot,
      detached: true,
      stdio: TEST_MODE && process.env.CODEX_BROWSER_EDGE_DEBUG === "1" ? ["ignore", "ignore", "inherit"] : "ignore",
      windowsHide: true,
      env: {
        ...process.env,
        CODEX_BROWSER_RUNTIME: selection.runtime,
        CODEX_BROWSER_AUTOSTART: "1",
        CODEX_BROWSER_SHOW_CONTROL_CENTER: TEST_MODE ? "0" : "1",
      },
    });
  } catch (error) {
    lastLaunchError = `Failed to start the ${selection.runtime} runtime: ${error instanceof Error ? error.message : String(error)}`;
    throw new Error(lastLaunchError);
  }
  ...
  child.once("error", (error) => {
    ...
    lastLaunchError = `The ${selection.runtime} runtime failed to start: ${error.message}`;
  });
...
async function callDesktop(method: string, params: Record<string, unknown> = {}): Promise<unknown> {
  ...
  try {
    ...
    if (TEST_MODE && process.env.CODEX_BROWSER_AUTOSTART_TEST !== "1") throw firstError;
  }

  launchDesktop();
  let lastError: unknown;
  // Match the desktop launcher's bounded 20-second cold-start window. Edge can
  // legitimately take longer than eight seconds after extraction or update.
...
const server = new McpServer({ name: "codex-browser", version: MCP_SERVER_VERSION });
...
registerTool("browser_storage_summary", "Read a safe browser storage summary containing only counts, approximate sizes, recovery state, and check time. Cookie names, values, storage contents, passwords, profile paths, and authorization data are never returned.", {
  tabId: tabIdSchema.optional(),
}, "browser.storage_summary");
registerTool("browser_profile_status", "Read the dedicated Codex Browser profile ownership and availability status without returning its filesystem path or browser data.", {}, "browser.profile_status");
registerTool("browser_confirmation_status", "Read sanitized high-risk action confirmation status. Codex cannot approve confirmations; approval is available only in the visible Electron control center.", {
  confirmationId: z.string().min(1).max(160).optional(),
}, "browser.confirmation_status");
registerTool("browser_confirmation_respond", "Deny a pending high-risk browser action. This MCP tool cannot approve an action; approval requires the user in the visible Electron control center.", {
  confirmationId: z.string().min(1).max(160),
  response: z.literal("deny"),
}, "browser.confirmation_respond");
registerTool("browser_grants_list", "List short-lived, origin- and category-scoped browser grants without exposing page content or credentials.", {}, "browser.grants_list");
registerTool("browser_grant_revoke", "Revoke one temporary browser grant by opaque ID. This cannot create or broaden a grant.", {
  grantId: z.string().min(1).max(160),
}, "browser.grant_revoke");
...
registerTool("browser_wait", "Wait for a page load, idle state, URL substring, visible text, or CSS selector without repeated polling.", {
  condition: z.enum([
    "load", "idle", "url", "text", "selector", "text_gone", "url_changed",
    "url_contains", "element_visible", "element_gone", "download", "dialog",
  ]),
...
registerTool("browser_request_assistance", "Ask the user to complete a browser step that Codex cannot or must not perform. The desktop browser displays the request visibly.", {
  kind: z.enum(["credential", "verification", "challenge", "passkey", "consent", "file_selection", "permission", "certificate", "manual_action"]),
