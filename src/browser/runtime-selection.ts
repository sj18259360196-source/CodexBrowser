export type ConfiguredBrowserRuntime = "external-edge" | "edge-extension" | "electron-legacy";
...
export function resolveBrowserRuntime(environment: NodeJS.ProcessEnv = process.env): BrowserRuntimeSelection {
  ...
  if (configured === "edge-extension") return { runtime: "edge-extension" };
  ...
  throw new Error("CODEX_BROWSER_RUNTIME must be external-edge, edge-extension, or electron-legacy.");
}
